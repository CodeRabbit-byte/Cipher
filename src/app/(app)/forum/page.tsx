...
export default async function ForumIndexPage() {
  ...
  const engagements = await prisma.engagement.findMany({
    ...
    select: {
      ...
      clientName: true,
      status: true,
      _count: { select: { observations: true, findings: true } },
    },
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Generating Forum</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Pick an engagement to open its AI forum
          </p>
        </div>
      </div>
      ...
      {engagements.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {engagements.map((eng) => (
            <Link
              key={eng.id}
              href={`/engagements/${eng.id}/forum`}
              className="group flex items-start gap-4 rounded-lg border p-4 hover:bg-muted/30 hover:shadow-sm transition-all"
            >
              <span className="mt-0.5 rounded-md bg-muted p-2 group-hover:bg-primary/10 transition-colors shrink-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {eng._count.observations} obs · {eng._count.findings} findings
                </p>
              </div>
            </Link>
          ))}
